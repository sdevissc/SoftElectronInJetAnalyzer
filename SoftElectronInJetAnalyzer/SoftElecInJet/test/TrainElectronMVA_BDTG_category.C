...
void TrainElectronMVA_BTDG_NonTrigV1_Cat1_2012() {
  ...
  TMVA::Tools::Instance();
  TFile* outputFile = TFile::Open("SoftElectronMVA__BDTG_2_2_0.root", "RECREATE");
  TMVA::Factory *factory = new TMVA::Factory("MVA", outputFile, "!V:!Silent");
  TFile* input = TFile::Open("./RecoToGenFill_QCD_80_170.root");  
 ...
 // factory->AddVariable("PreShowerOverRaw",'F');
  ...
  factory->PrepareTrainingAndTestTree("pt>10 && abs(eta)>1.4","pt>10 && abs(eta)>1.4","nTrain_Signal=0:nTrain_Background=0:SplitMode=Block:!V" );
  

  
  factory->BookMethod(TMVA::Types::kBDT, "BDT_2_2_0", "!H:!V:NTrees=2000:BoostType=Grad:Shrinkage=0.10:!UseBaggedGrad:nCuts=2000:nEventsMin=100:NNodesMax=5:UseNvars=4:PruneStrength=5:PruneMethod=CostComplexity:MaxDepth=6" );
