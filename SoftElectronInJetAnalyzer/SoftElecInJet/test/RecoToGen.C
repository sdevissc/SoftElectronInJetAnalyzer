...
class HistoMaker
{
  public:
	...
	TH1F* ForIntegral;
	int nbin[34];
	float xmin[34],xmax[34],ymin[34],ymax[34];
	TString VarName[34];
};
...
//	cout<<"define the binning"<<endl;
	xmin[0]=0;	xmax[0]=6;	ymin[0]=0;	ymax[0]=0.2;
        ...
        xmin[19]=0;     xmax[19]=1;      	ymin[19]=0;      ymax[19]=0.4;
        ...
        xmin[29]=0;  	xmax[29]=0.1;     	ymin[29]=0;      ymax[29]=0.4;
        xmin[30]=0.1;  xmax[30]=1.0;    	ymin[30]=0;      ymax[30]=0.4;
        xmin[31]=0;     xmax[31]=0.02;     	ymin[31]=0;      ymax[31]=1;
        xmin[32]=0;  	xmax[32]=1;    		ymin[32]=0;      ymax[32]=1;
        xmin[33]=-1;     xmax[33]=1;             ymin[33]=0;      ymax[33]=1;
...
class SLPlotter                   // begin declaration of the class
{
  public:                    // begin public section
	...
	void MakeEffvsEffPlot(TString &process);
...
SLPlotter::SLPlotter(const TString &input,const TString &tag, int thecond)
{
	ProcessFile = input;
	TheTag=tag;
	for(int u=0;u<5;u++)for(int v=0;v<2;v++){
...
//	GetEscOverPGen(ProcessName);
	...
	for(int k=0;k<34;k++){
		...
		if(k==31 || k==32 || k==20 || k==16 || k==17 || k==22 || k==30){gPad->SetLogy(1);}
                else{gPad->SetLogy(0);}
...
//		cout<<"legend drawn"<<endl;
        	...
        	if(k==0){can->Print(Form(ProcessName+"_var_cond_%d.pdf(",condInput));}	
		if(k>0 && k<33){can->Print(Form(ProcessName+"_var_cond_%d.pdf",condInput));}
		if(k==33){can->Print(Form(ProcessName+"_var_cond_%d.pdf)",condInput));}
//		cout<<"ok"<<endl;
		SetStyle(Object[0][0]->var[k],1,3004 ,  0.0001,0.2);
		SetStyle(Object[3][0]->var[k],2,0, 0.0001,0.2);
                Object[0][0]->var[k]->Draw("");
                Object[3][0]->var[k]->Draw("same");
                legend2->Draw();
                if(k==0)can->Print(Form("Summary_"+ProcessName+"_var_cond_%d.pdf(",condInput));
		if(k>0 && k<33)can->Print(Form("Summary_"+ProcessName+"_var_cond_%d.pdf",condInput));
		if(k==33)can->Print(Form("Summary_"+ProcessName+"_var_cond_%d.pdf)",condInput));
		delete can;
	}
	delete legend;
	delete legend2;
...
void SLPlotter::TheFill(HistoMaker *obj,EventPurity *Evt){
	...
	obj->var[28]->Fill(Evt->eta);
        obj->var[29]->Fill(Evt->spp);
        obj->var[30]->Fill(Evt->R9);
        obj->var[31]->Fill(Evt->IoEmIoP);
        obj->var[32]->Fill(Evt->PreShowerOverRaw);	
        obj->var[33]->Fill(Evt->mva_e_pi);
...
bool  SLPlotter::Condition(int cond,EventPurity *Evt){
	...
	if(cond==9 && Evt->mva_e_pi>-0.1)return true;	
	if(cond==10 && Evt->mva_e_pi>0.5)return true;
	if(cond==11 && Evt->mva_e_pi>0.7)return true;
	if(cond==12 && Evt->mva_e_pi>0.9)return true;
	if(cond==13 && Evt->mva_e_pi>0.95)return true;
	if(cond==14 && Evt->mva_e_pi>0.95 && Evt->fromConversion==0)return true;
...
void SLPlotter::FillHistos(){
	for(int ientry=0; ientry<nentries; ientry++) {
	   ...
	   //     	bool isfromX=(Evt->origin==0);	
			...
			bool isBackground=abs(Evt->pdgId)!=11;	
...
void SLPlotter::MakeEffvsEffPlot(TString &process){
	...
	const int Nbin2D=90;
        ...
        for(int k=0;k<Nbin2D;k++){
                float fr1=Object[0][0]->var[33]->Integral(k+1,Nbin2D)/Object[0][0]->var[33]->Integral();
                float fr2=Object[4][0]->var[33]->Integral(k+1,Nbin2D)/Object[4][0]->var[33]->Integral();
		cout<<"bin k:"<<Object[0][0]->var[33]->Integral(k+1,Nbin2D)<<" "<<Object[0][0]->var[33]->Integral()<<" "<<Object[4][0]->var[33]->Integral(k+1,Nbin2D)<<" "<<Object[4][0]->var[33]->Integral()<<endl;
                x_ged_all[k]=fr1;
                y_ged_all[k]=fr2;
        }
        ...
        gr_ged_all->GetYaxis()->SetTitle("mistag rate");

//        legend->Clear();
//        legend->AddEntry(gr_ged_all,"GED new mva_e_pi","p");
     //   legend->Draw();
        can->Print(ProcessName+"effvsmistagrate_GEDvsPF.pdf");
