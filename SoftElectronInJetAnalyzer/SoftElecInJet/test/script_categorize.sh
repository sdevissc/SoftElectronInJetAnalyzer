...
for pu in "${PUrange[@]}"
	...
	for pt in "${ptrange[@]}"
		...
		for eta in "${etarange[@]}"
		do
			echo "BDT_${indpt_1}_${indeta_1}_${indpu_1}: ${pt} \&\& ${eta} \&\& ${pu}"
			let "indeta_1 += 1";
		done
		let "indpt_1 += 1"
	done
		...
		for pu in "${PUrange[@]}"
			...
			do
				...
				do
					...
					echo "->${eta}"
					string="${pt} \&\& ${eta} \&\& ${pu}"
					suffix=${indpt}_${indeta}_${indpu}_${indfile}
					echo "-->$string"
					sed "s/YYY/${string}/g"  TrainElectronMVA_BDTG_automated.C > temp_${suffix}
					sed "s/XXX/${suffix}/g"  temp_${suffix} > temp_out_${suffix}
					sed "s/ZZZ/${file}/g"  temp_out_${suffix} > temp_out2_${suffix}
					sed "s/XXXX/${suffix}/g"  TMVAGui_automated.C > TMVAGui_${suffix}.C
					rm temp_${suffix}
					rm temp_out_${suffix}	
					mv temp_out2_${suffix} TrainElectronMVA_BDTG_category.C
					root -l -b <<-EOF
