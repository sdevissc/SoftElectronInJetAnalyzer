...
void TrainElectronMVA_BTDG_NonTrigV1_Cat1_2012() {
  ...
  TMVA::Tools::Instance();
  TFile* outputFile = TFile::Open("ElectronMVA_NonTrig_BDTG_ISO.root", "RECREATE");
  ...
  factory->AddVariable("IoEmIoPMean", 'F');
  factory->AddVariable("eleEoPout", 'F');

  // isolation
  factory->AddVariable("relIsoRing1", 'F');
  factory->AddVariable("relIsoRing2", 'F');
  factory->AddVariable("relIsoRing3", 'F');
  factory->AddVariable("relIsoRing4", 'F');
  factory->AddVariable("relIsoRing5", 'F');
  factory->AddVariable("relIsoRing6", 'F');
  ...
  //factory->AddSpectator( "weightrho");


  //Split using parity of event number
  TFile* inputSignalAll = TFile::Open("/data/benedet/multivariateEleID/DataForTraining/Emanuele2012DATA/12Nov/newtmva_Em_12Nov_MC.HZZ.All.root");   
  TFile* inputBkgTraining = TFile::Open("/data/benedet/multivariateEleID/DataForTraining/Emanuele2012DATA/12Nov/newtmva_Em_12Nov_W1J.Fakes.All.root");
  TFile* inputBkgTesting = TFile::Open("/data/benedet/multivariateEleID/DataForTraining/Emanuele2012DATA/12Nov/newtmva_Em_12Nov_Z1J.Fakes.All.root");
  ...
  factory->SetSignalWeightExpression("weightrho");
  factory->SetBackgroundWeightExpression("weightrho");
